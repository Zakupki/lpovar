        ...
        <div data-block="sjstp3" class="control-group control-group-multiple product-info">
            ...
            <?php foreach($BlogDish as $i => $item) { /** @var $item ProductInfo */?>
                ...
                <div class="controls controls-multiple-row">
                    <?php echo $form->dropDownList($item, "[{$i}]dish_id", CHTML::listData(Dish::model()->findAllByAttributes(array('dishgroup_id'=>1)),'id','title'), array(
...
<script type="text/html" id="sjstp3">
    <?php echo $form->dropDownList($BlogDish[0], "[<%=idx%>]dish_id", CHTML::listData(Dish::model()->findAllByAttributes(array('dishgroup_id'=>1)),'id','title'), array(
        'value' => '',
        'encode' => false,
        'class' => 'span3 validate-unique',
        'placeholder' => Yii::t('backend', 'Dish'),
