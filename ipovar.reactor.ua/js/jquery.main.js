		...
		if(sidebar.size()){
			scrollH=function(){
				if($(window).scrollTop()>holder.offset().top){
					sidebar.css({'position':'fixed','top':0,'z-index':19,'left':(holder.offset().left-$(window).scrollLeft()+holder.width())-sidebar.width()});
				} else{
					sidebar.attr('style','');
				}
				if($(window).scrollTop()+sidebar.height()>content.offset().top+content.height()){
					sidebar.css({'position':'relative','top':content.height()-sidebar.height()-20,'z-index':19,'left':0});
				}
			}
		}
		$(window).scroll(scrollH);
...
function initLoginFormValidation(){
	var form=$('#login form'),
		input=form.find('input:LoginForm[email],input:LoginForm[password]');
	form.submit(function(e){
		if(!form.hasClass('inProgress')){
			form.addClass('inProgress');
			$.ajax("/site/login/").done(function(data){
...
function initSettings () {
	...
	$('.settings-form .green-btn input').click(function(e){
		var _text = $(this).parents('.row').find('.input-holder input').val(),
			_text1 = $(this).parents('.row').find('textarea').val();
		var fieldname = $(this).parents('.row').find('.input-holder input').attr('name'),
			fieldname1 = $(this).parents('.row').find('textarea').attr('name');
		...
		$.post('/site/updatesettings/', postkey+'='+postval, 'json').done(function(data){
			if(data.error){
            alert(data.status); 
            }
        });
...
function initLogin () {
	$('#nav li.login a, #login_link').click(function(e){
		$('#login').hide();
		$('.bg').height(popups());
		$('#login').css('left', '0').fadeIn(200);
        if ($('#login .popup').height() < $(window).height()) {
			$('#login .popup').css({top:($(window).height()/2-$('#login .popup').outerHeight()/2+$(window))});
		}
		e.preventDefault();
	});
	$('.bg, .popup .close, .popup .close-popup').click(function(e){
	...
	$('#login .forgot').click(function(e){
		...
		if ($('#remind .popup').height() < $(window).height()) {
			$('#remind .popup').css({top:($(window).height()/2-$('#remind .popup').outerHeight()/2+$(window))});
		}
		e.preventDefault();
	});
	$('.promo-code-opener').click(function(e){
		...
		if ($('#promo-code .popup').height() < $(window).height()) {
			$('#promo-code .popup').css({top:($(window).height()/2-$('#promo-code .popup').outerHeight()/2+$(window))});
		}
		e.preventDefault();
	});
}
...
jQuery.fn.fadeGallery = function(_options){
	...
	return this.each(function(){
		...
		function switchSlide() {
            ...
            _slides.eq(_prevIndex).animate({opacity:0}, _duration);
            _slides.eq(_currentIndex).animate({opacity:1}, _duration);
